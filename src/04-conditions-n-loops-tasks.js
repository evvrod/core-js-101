...
function findFirstSingleChar(str) {
  for (let i = 0; i < str.length; i += 1) {
    if (!(str.slice(0, i) + str.slice(i + 1)).includes(str[i])) return str[i];
  }
  return null;
}
...
function isCreditCardNumber(ccn) {
  ...
  let sum = 0;
  const ost = String(ccn).length % 2;
  [...strCCN].forEach((el, ind) => {
    let digit = Number(el);
    if (ind % 2 === ost) {
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] in pair) {
      arr.push(str[i]);
    } else if (pair[arr.pop()] !== str[i]) return false;
  }
  return arr.length === 0;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  const trArr = arr[0].map((col, c) => arr.map((row, r) => arr[r][c]));
  let str = '';
  for (let i = 0; i < trArr.length; i += 1) {
    if (new Set(trArr[i]).size === 1) {
      str += `${trArr[i][0]}/`;
    }
  }
...
function getMatrixProduct(m1, m2) {
  const matrix = Array(m1.length).fill(0).map(() => Array(m2[0].length).fill(0));
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < 3; i += 1) {
    ...
    for (let k = 0; k < 3; k += 1) {
      sumRow += matrix[i][k];
      sumCol += matrix[k][i];
      sumDiagon += matrix[k][k];
      sumInvDiagon += matrix[k][2 - k];
    }
    if (sumRow === 3 || sumCol === 3 || sumDiagon === 3 || sumInvDiagon === 3) return 'X';
    if (sumRow === -3 || sumCol === -3 || sumDiagon === -3 || sumInvDiagon === -3) return '0';
  }
  return undefined;
}
